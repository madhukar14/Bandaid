...
Band.prototype.init = function(){	
	...
	strr = str.split("?")[1];
	 ...
	 var nameval = name_key_val.split("=")[1].replace(/%20/g," ");
	 var typeval = type_key_val.split("=")[1].replace(/%20/g," ");
	 nameval = nameval.replace(/\s{2,}/g,' ');;
	 typeval = typeval.replace(/\s{2,}/g,' ');;
	 console.log(nameval);
	 console.log(typeval);
	 this.band_name = nameval;
	 this.band_type = typeval;	 
	$.get(SERVER+"/DBAjaxReq.php",{func:"return_band_data", data:{name:this.band_name,type:this.band_type}},$.proxy(this.userDetailsAvailable,this),type="json");
	$.get(SERVER+"/DBAjaxReq.php",{func:"band_reviews", data:{band_name:this.band_name,band_type:this.band_type}},$.proxy(this.bandReviewsAvailable,this),type="json");
	this.addHeader();
...
Band.prototype.userDetailsAvailable = function(data){
	...
	this.userDetails.hasLiked = data.data.has_liked;
	if(this.userDetails.hasLiked){
		$('.band-like').addClass("band-liked");
		$('.band-like').val("Unlike");
	}
	if($('.band-name').parent().length != 0){
		$('.band-name').html(this.userDetails.band_name);
	}
...
Band.prototype.setUp = function(data){
	...
	$('.band-image').attr("src",data.pic_url);
...
Band.prototype.getReviewTmpl = function(item){
	...
	'<span class="what-r">'+ item.review_text+'</span><br>'+'<span class="who-r">- by '+item.reviewed_by+' - '+item.reviewed_on+'</span>';
	return tmpl;
...
Band.prototype.bandLiked = function(){
	...
	$.get(SERVER+"/"+"DBAjaxReq.php",{func:"band_like_unlike",data:{bname: this.band_name}},type="json");
};
...
Band.prototype.musicClicked = function(){
	$.get(SERVER+"/DBAjaxReq.php",{func:"band_music",data:{band_type:this.band_type,band_name: this.band_name}},$.proxy(this.musicAvailable,this),type="json");	
};
...
Band.prototype.reviewBand = function(){
	...
	dialog.show();	
	dialog.setTitle("Review Band - "+this.band_name);
	...
	$('.review-submit').click(function(){
		...
		$.get(SERVER+"/"+"DBAjaxReq.php",
			{func:"band_review", data:{bname: that.band_name, btype: that.band_type, review_text: rText }},
			type="json");
		dialog.close();
	});
};
